...
class TurtleActionClient(Node):
    ...
    def feedback_callback(self, feedback_msg):
        ...
        self.get_logger().info(f'[Client]. Got feedback: {feedback.odom}')

    def goal_response_callback(self, future):
        goal_handle = future.result()
        if not goal_handle.accepted:
            self.get_logger().info(f'[Client]. Goal declined')
            return

        self.get_logger().info(f'[Client]. Goal accepted')
    ...
    def get_result_callback(self, future):
        ...
        if status == 4:
            self.get_logger().info(f'[Client]. Goal succeeded: {result}')
            self.send_next_goal()
        else:
            self.get_logger().info(f'[Client]. Goal failed: {status}')
